...
public class NewMusicPlayer : MonoBehaviour {
    static NewMusicPlayer instance = null;     //	Singleton used to load the music player
    ...
    public AudioClip winClip;               //Clip to celebrate your victory over the galactic empire
    ...
    void OnLoadCallBack(Scene scene, LoadSceneMode sceneMode)
    {
        ...
        {
            if (scene.buildIndex == 4) //If the player lose, play loseClip
            {
                music.clip = endClip;
            }
            else if (scene.buildIndex == 3) //If the player wins, play winClip
            {
                music.clip = winClip;
            }
            else
            {
                music.clip = gameClip; //Every other scene, play MosEisley's theme
            }
        }
